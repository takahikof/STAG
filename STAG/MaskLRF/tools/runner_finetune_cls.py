...
def set_parameter_grads( model, logger ):

    # list_finetune_target = [ "cls_head_finetune" ]
    list_finetune_target = [ "cls_head_finetune", "addparam_" ]

    # #######
    # # tune LayerNorm
    # blocks = model.module.blocks.token_modulation_blocks
    # norms = ["norm1", "norm2"]
    # for b in blocks:
    #     for n in norms:
    #         name = "blocks.blocks." + b + "." + n + ".weight"
    #         list_finetune_target.append( name )
    #         name = "blocks.blocks." + b + "." + n + ".bias"
    #         list_finetune_target.append( name )
    # #######

    print_log('Finetuned parameters:', logger = logger)
    for name, param in model.module.named_parameters():
        # print( name )
        is_target = False
        for target in list_finetune_target:
            if( target in name ):
                is_target = True
        if( is_target ):
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # build dataset
    train_sampler, train_dataloader, _ = builder.dataset_builder(args, config.dataset.train)
    ...
    ###############
    set_parameter_grads( base_model, logger )
    ###############
