...
class SH15_Nonrigid(data.Dataset):
    def __init__(self, config):
        ...
        if self.subset == 'train':
            h5 = h5py.File(os.path.join(self.root, 'sh15nr_1024pts_withnormal_train.h5'), 'r')
            self.points = np.array(h5['data']).astype(np.float32)
            self.labels = np.array(h5['label']).astype(int)
            h5.close()
        elif self.subset == 'test':
            h5 = h5py.File(os.path.join(self.root, 'sh15nr_1024pts_withnormal_test.h5'), 'r')
            self.points = np.array(h5['data']).astype(np.float32)
            self.labels = np.array(h5['label']).astype(int)
            h5.close()
        else:
    ...
    def __getitem__(self, index):
        ...
        return 'SH15_Nonrigid', 'sample', (point_set, label)
