...
class FG3D(data.Dataset):
    def __init__(self, config):

        self.coarse_category = config.COARSE_CATEGORY # airplane/car/chair
        ...
        if self.subset == 'train':
            h5 = h5py.File(os.path.join(self.root, 'fg3d_'+self.coarse_category+'_1024pts_withnormal_train.h5'), 'r')
            self.points = np.array(h5['data']).astype(np.float32)
            self.labels = np.array(h5['label']).astype(int)
            h5.close()
        elif self.subset == 'test':
            h5 = h5py.File(os.path.join(self.root, 'fg3d_'+self.coarse_category+'_1024pts_withnormal_test.h5'), 'r')
            self.points = np.array(h5['data']).astype(np.float32)
            self.labels = np.array(h5['label']).astype(int)
            h5.close()
        else:
    ...
    def __getitem__(self, index):
        ...
        return 'FG3D_'+self.coarse_category, 'sample', (point_set, label)
